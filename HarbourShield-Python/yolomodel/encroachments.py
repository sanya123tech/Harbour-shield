...
# Indices for boat-related classes
boat_classes = ['boat', 'ship', 'kayak']
boat_class_ids = [classes.index(cls) for cls in boat_classes if cls in classes]
...
for i in range(len(boxes)):
    if i in indexes and class_ids[i] in boat_class_ids:
        x, y, w, h = boxes[i]
        ...
        detected_objects.append({
            "label": label,
            "timestamp": current_time,  # Use timestamp instead of confidence
            "box": {"x": x, "y": y, "w": w, "h": h}
        })
...
try:
    doc_ref = db.collection("detected_objects").add({
